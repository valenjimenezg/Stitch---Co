...
class InvoiceController extends Controller
{
    public function descargarFactura($id)
    {
        ...
        $query = Venta::with(['detalles.variante.producto', 'user']);
        ...
        if (!in_array($venta->estado, ['completado', 'entregado'])) {
        ...
        $qrData = \Illuminate\Support\Facades\URL::signedRoute('invoice.public', ['id' => $venta->id]);
        ...
        $tasa_actual = $venta->tasa_bcv_aplicada ?? bcv_rate();

        return view('factura', ['orden' => $venta, 'qrCode' => $qrCode, 'tasa_actual' => $tasa_actual]);
    }
    ...
    public function descargarFacturaPublica(\Illuminate\Http\Request $request, $id)
    {
        ...
        $venta = Venta::with(['detalles.variante.producto', 'user'])->findOrFail($id);

        if (!in_array($venta->estado, ['pagado', 'procesando', 'completado', 'entregado'])) {
    ...
    {
        $orden = Venta::with(['detalles.variante.producto', 'user'])->findOrFail($id);
